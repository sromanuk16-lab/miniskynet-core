// MiniSkynet Core v2.4 — identity, goals, plan.
// Single Cloudflare Worker entry: Telegram router, KV memory/tasks/proposals, memory quality gate.

const VERSION = "v2.4-identity-goals-plan-2026-07-03";
...
const DEFAULT_SELF = {
  name: "MiniSkynet Core v2.4",
  role: "личный инженерный агент Сергея в Telegram на Cloudflare Worker",
  identity: "Я помогаю вести MiniSkynet и другие проекты, держу память, цели, план и предлагаю маленькие безопасные шаги.",
...
const DEFAULT_GOALS = [
  ...
  { id: "g3", text: "Планировать развитие маленькими безопасными патчами.", status: "active" },
  { id: "g4", text: "Перед изменениями кода читать актуальные файлы и после деплоя проверять результат.", status: "active" }
];
const DEFAULT_PLAN = [
  { id: "p1", text: "Проверить /self, /goals, /plan и /next после деплоя v2.4.", status: "todo" },
  { id: "p2", text: "Добавить Project Knowledge для MiniSkynet после стабилизации целей.", status: "todo" },
  { id: "p3", text: "Добавить GitHub self-inspection только после чистой памяти и целей.", status: "todo" }
];
...
function formatSelf(s) {
  return ["🪪 Self:", `- version: ${VERSION}`, `- name: ${s.name}`, `- role: ${s.role}`, `- identity: ${s.identity}`, "- limits:", ...(s.limits || []).map(x => `  • ${x}`), "", "Изменить: /self_set текст", "Сбросить: /self_reset"].join("\n");
}
function formatGoals(goals) {
  return ["🎯 Goals:", ...(goals.length ? goals.map((g, i) => `${i + 1}. [${g.status || "active"}] ${g.text}`) : ["пусто"]), "", "Изменить: /goal_add текст | /goal_del номер | /goals_set цель1 | цель2 | /goals_reset"].join("\n");
}
function formatPlan(plan) {
  return ["🧭 Plan:", ...(plan.length ? plan.map((p, i) => `${i + 1}. [${p.status || "todo"}] ${p.text}`) : ["пусто"]), "", "Изменить: /plan_set шаг1 | шаг2 | /plan_add текст | /plan_done номер | /plan_reset"].join("\n");
}
function nextStepText(tasks, plan, goals) {
  const t = visibleTasks(tasks).find(x => ["todo", "retry_wait", "doing", "pending", "active"].includes(String(x.status || "todo")));
  if (t) return [`⏭ Next:`, `Источник: tasks`, `Шаг: ${t.title}`, `Почему: это активная задача с приоритетом p${t.priority || 4}.`].join("\n");
  const p = plan.find(x => String(x.status || "todo") !== "done");
  if (p) return [`⏭ Next:`, `Источник: plan`, `Шаг: ${p.text}`, `Почему: это первый незавершённый шаг плана.`].join("\n");
  ...
  return "⏭ Next: нет активных задач, плана и целей. Добавь цель через /goal_add.";
}
...
async function costReport(env, cfg) {
  ...
  return ["💸 CostGuard:", `- циклы сегодня: ${st.cycles} / ${cfg.maxCyclesPerDay}`, `- tokens: in=${st.input_tokens}, out=${st.output_tokens}`, `- cost: ~$${Number(st.cost_usd || 0).toFixed(6)} / $${cfg.maxDailyCostUsd}`].join("\n");
}
...
const QUALITY_GOOD_TERMS = ["miniskynet", "skynet", "v2", "cloudflare", "worker", "telegram", "openrouter", "kv", "github", "wrangler", "deploy", "memory", "tasks", "costguard", "status", "clean", "hygiene", "команда", "деплой", "память", "задачи", "репозитор", "патч", "ошибка", "правило", "архитектур", "goal", "plan"];
...
function memoryQuality(m) {
  ...
  if (/развит|улучш|полезн|следующ/.test(text) && !/\/[a-z_]+|cloudflare|github|telegram|openrouter|kv|worker|wrangler|deploy/i.test(text)) score -= 22;
...
async function saveMemory(env, raw, agent = "core") {
  ...
  if (!m.lesson || !memoryQuality(m).keep) return null;
...
async function addTask(env, title, priority = 4, force = true) {
  const task = { id: uid("task"), title: clip(title, 300), priority, status: "todo", created_at: now() };
  if (!force && isStaleTask(task)) return null;
...
function cleanState(tasks, memories) {
  const cleanTasks = visibleTasks(tasks);
  const cleanMemories = [];
  const seen = new Set();
  for (const m of memories) {
    const k = compact(m.lesson);
    if (isStaleMemory(m) || !k || seen.has(k)) continue;
    seen.add(k);
    cleanMemories.push(m);
  }
  return { cleanTasks, cleanMemories, removedTasks: tasks.length - cleanTasks.length, removedMemories: memories.length - cleanMemories.length };
}
function seedMemory() {
  return normalizeMemory({ signal: "MiniSkynet Core v2.4 identity/goals/plan активен.", lesson: "Self, goals и plan теперь можно менять прямо из Telegram-команд без редеплоя кода.", action: "Использовать /self_set, /goal_add, /goals_set, /plan_set и /next для управления направлением агента.", check: "Проверять /self, /goals, /plan и /next после изменений.", boundary: "Эти настройки живут в KV и могут быть изменены владельцем в Telegram.", status: "fact", score: 95 }, "identity");
}
function cleanPreviewText(tasks, memories) {
  const c = cleanState(tasks, memories);
  return ["🧹 KV preview:", `- задач: ${tasks.length} → ${c.cleanTasks.length}`, `- памяти: ${memories.length} → ${c.cleanMemories.length}`, "Применить: /clean_apply"].join("\n");
}
async function applyClean(env) {
  const [tasks, memories] = await Promise.all([getTasks(env), getMemories(env)]);
  const c = cleanState(tasks, memories);
  const finalMemories = [...c.cleanMemories];
  if (!finalMemories.some(m => compact(m.lesson) === compact(seedMemory().lesson))) finalMemories.push(seedMemory());
  await Promise.all([saveTasks(env, c.cleanTasks), saveMemories(env, finalMemories)]);
  return { ...c, beforeTasks: tasks.length, beforeMemories: memories.length, cleanMemories: finalMemories };
}
function memoryScoreText(memories) {
  ...
  return ["🧠 Memory Quality Gate:", `- всего: ${rows.length}`, `- проходит gate: ${keep}`, `- слабая/generic: ${rows.length - keep}`, `- средний score: ${avg}/100`, "", ...rows.slice(-10).map(x => `- q${x.q.score} ${x.q.keep ? "✅" : "🗑"} [${x.m.status}] ${clip(x.m.lesson, 95)}`)].join("\n");
}
async function memoryPruneApply(env) {
  const memories = await getMemories(env);
  const kept = visibleMemories(memories);
  await saveMemories(env, kept);
  return { before: memories.length, after: kept.length, removed: memories.length - kept.length };
}
function memoryPrunePreviewText(memories) {
  const weak = memories.filter(m => !memoryQuality(m).keep);
  return ["🧹 Memory prune preview:", `- памяти сейчас: ${memories.length}`, `- будет удалено: ${weak.length}`, ...weak.slice(0, 12).map(m => `- q${memoryQuality(m).score} ${clip(m.lesson || m.signal, 120)}`), "", "Применить: /memory_prune_apply"].join("\n");
}
function selectRelevant(memories, query, n = 6) {
  const q = compact(query).split(/\W+/).filter(Boolean);
  return memories.map(m => {
    const hay = memoryText(m);
    const hits = q.reduce((a, w) => a + (hay.includes(w) ? 1 : 0), 0);
    return { m, score: hits * 30 + memoryQuality(m).score + (m.status === "rule" ? 30 : m.status === "fact" ? 15 : 0) };
...
async function chat(env, cfg, prompt) {
  ...
  const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: cfg.modelCheap, temperature: 0.3, max_tokens: maxOut, messages: [{ role: "system", content: "Ты MiniSkynet Core v2.4. Отвечай по-русски, коротко, честно. Для JSON-заданий возвращай только валидный JSON." }, { role: "user", content: prompt }] })
  });
...
function thinkPrompt({ text, tasks, memories, self, goals, plan }) {
  return [
    "Ты — MiniSkynet Core v2.4, личный инженерный агент Сергея.",
    ...
    "Память сохраняй только если это конкретный факт/правило/ошибка/архитектурный вывод о проекте. Не сохраняй generic-фразы. Если нечего сохранить — memory_artifact=null.",
...
async function runThink(env, cfg, chatId, text) {
  const [tasks, allMem, self, goals, plan] = await Promise.all([getTasks(env), getMemories(env), getSelf(env), getGoals(env), getPlan(env)]);
  const memories = selectRelevant(visibleMemories(allMem), text, 8);
  let response;
  try { response = await chat(env, cfg, thinkPrompt({ text, tasks, memories, self, goals, plan })); }
...
async function reflect(env, cfg, chatId = null) { const r = await memoryPruneApply(env); const report = `🌙 Рефлексия v2.4:\n- память было: ${r.before}\n- стало: ${r.after}\n- удалено слабой/generic: ${r.removed}`; if (chatId) await send(cfg, chatId, report); return report; }
...
async function handleCommand(env, cfg, msg) {
  ...
  if (command === "/start") { const b = await getBrain(env); b.owner_chat_id = chatId; await saveBrain(env, b); return await send(cfg, chatId, `✅ MiniSkynet Core v2.4 проснулся.\n\nTelegram user id: ${userId}\nChat id: ${chatId}\n\n/help — команды`); }
  if (command === "/help") return await send(cfg, chatId, HELP);
  if (command === "/status") { const [b, tasks, mem, props, goals, plan] = await Promise.all([getBrain(env), getTasks(env), getMemories(env), getProposals(env), getGoals(env), getPlan(env)]); return await send(cfg, chatId, ["📡 MiniSkynet v2.4 status", `- version: ${VERSION}`, `- alive: ${b.alive_enabled}`, `- циклов всего: ${b.stats?.cycles_total || 0}`, `- задачи: active=${visibleTasks(tasks).length}, hidden=${tasks.length - visibleTasks(tasks).length}`, `- память: clean=${visibleMemories(mem).length}, weak=${mem.length - visibleMemories(mem).length}`, `- goals: ${goals.length}`, `- plan steps: ${plan.length}`, `- proposals pending: ${props.filter(p => p.status === "pending").length}`, `- модель: ${cfg.modelCheap}`].join("\n")); }

  if (command === "/self") return await send(cfg, chatId, formatSelf(await getSelf(env)));
  if (command === "/self_set") { if (!args) return await send(cfg, chatId, "Напиши: /self_set Я — ..."); const s = await getSelf(env); s.identity = clip(args, 1200); await saveSelf(env, s); await saveMemory(env, { signal: "Self MiniSkynet изменён владельцем в Telegram.", lesson: "Identity/Self ядра MiniSkynet v2.4 редактируется через /self_set без редеплоя.", action: "Использовать новый self в /think, /plan и /next.", check: "Проверить командой /self.", status: "fact", score: 95 }, "identity"); return await send(cfg, chatId, "✅ Self обновлён. Проверь: /self"); }
  ...
  if (command === "/next") { const [tasks, plan, goals] = await Promise.all([getTasks(env), getPlan(env), getGoals(env)]); return await send(cfg, chatId, nextStepText(tasks, plan, goals)); }
...
async function aliveTick(env, cfg) {
  ...
  await runThink(env, cfg, b.owner_chat_id, "Автоцикл: проверь self/goals/plan и предложи один полезный следующий шаг. Не делай код сам.");
  return { status: "done" };
}
