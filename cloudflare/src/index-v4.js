const VERSION = "v4.7-proposal-specific-verify-2026-07-03";
...
async function tg(c, method, body) {
  ...
  const r = await fetch(`https://api.telegram.org/bot${c.telegramToken}/${method}`, {
    method: "POST",
...
function health() {
  return {
    ...
    scoped_help_insert: true,
    proposal_specific_verify: true,
    apply_rebuild: true,
    owner_guard: "fixed"
  };
}
async function publicHealth(c) {
  ...
  try {
    const base = String(c.workerUrl || DEFAULT_WORKER_URL).replace(/\/$/, "");
    const r = await fetch(`${base}/health?ts=${Date.now()}`, { headers: { "cache-control": "no-cache" } });
    const txt = await r.text();
    let data = null;
    try { data = JSON.parse(txt); } catch {}
    return { ok: r.ok && Boolean(data?.ok), http: r.status, version: data?.version || null, ms: Date.now() - started };
  } catch (e) {
    return { ok: false, http: 0, version: null, ms: Date.now() - started, error: String(e.message || e) };
  }
}
function b64Decode(x) {
  return new TextDecoder("utf-8").decode(Uint8Array.from(atob(String(x || "").replace(/\n/g, "")), ch => ch.charCodeAt(0)));
}
function b64Encode(x) {
  const b = new TextEncoder().encode(String(x || ""));
  let s = "";
  for (let i = 0; i < b.length; i += 0x8000) s += String.fromCharCode(...b.slice(i, i + 0x8000));
  return btoa(s);
}
...
async function ghFile(c, path) {
  ...
  const headers = { accept: "application/vnd.github+json", "user-agent": "MiniSkynet-v47" };
...
async function ghWrite(c, path, sha, content, message) {
  ...
  const p = safePath(path);
  if (!p) throw new Error("unsafe path");
  const r = await fetch(`https://api.github.com/repos/${c.repo}/contents/${pathUrl(p)}`, {
    method: "PUT",
    headers: { accept: "application/vnd.github+json", "content-type": "application/json", "user-agent": "MiniSkynet-v47", authorization: `Bearer ${c.githubToken}` },
    body: JSON.stringify({ message, content: b64Encode(content), sha, branch: c.branch })
  });
  const data = await r.json().catch(() => ({}));
...
function mainFromWrangler(txt) {
  const m = String(txt || "").match(/^main\s*=\s*["']([^"']+)["']/m);
  if (!m) return null;
  const rel = cleanPath(m[1]);
  return rel.startsWith("cloudflare/") ? rel : `cloudflare/${rel}`;
}
async function activeTarget(c) {
  const wr = await ghFile(c, "cloudflare/wrangler.toml");
  const main = mainFromWrangler(wr.content);
  if (!main) throw new Error("wrangler main not found");
  return { start: main, effective: await ghFile(c, main) };
}
function findProp(items, key) {
  const k = String(key || "").trim();
  return items.find(p => p.id === k || String(p.id || "").startsWith(k));
}

function fullDiff(path, oldContent, newContent, oldSha) {
  const a = String(oldContent || "").replace(/\n$/, "").split("\n");
...
function applyFullDiff(oldContent, diff) {
  const out = [];
  let h = false;
  for (const line of String(diff || "").split("\n")) {
    if (line.startsWith("@@ ")) { h = true; continue; }
    if (!h) continue;
...
function helpRange(content) {
  const text = String(content || "");
  const start = text.indexOf('if (command === "/help")');
  const end = start >= 0 ? text.indexOf('if (command === "/status")', start) : -1;
  return start >= 0 && end > start ? { start, end, block: text.slice(start, end) } : null;
}
function helpHas(content, visible) {
  const r = helpRange(content);
  return Boolean(r && r.block.includes(String(visible || "")));
}
function insertHelpLine(content, jsLine) {
  ...
  const r = helpRange(content);
  if (!r) throw new Error("help block not found");
  const anchors = [
  ...
  for (const anchor of anchors) {
    if (block.includes(anchor)) {
      block = block.replace(anchor, `      ${jsLine},\n${anchor}`);
...
function makePatch(p, file) {
  const spec = expectedForRequest(`${p.title || ""} ${p.request || ""}`);
  if (!spec) throw new Error("Нет безопасной deterministic operation. Используй: auto pipeline marker / confirmed marker / smoke marker / help stage.");
  if (helpHas(file.content, spec.visible)) return { already: true, summary: `${spec.visible} уже есть в /help`, expected_visible: spec.visible, content: file.content };
  return { already: false, summary: spec.summary, expected_visible: spec.visible, content: insertHelpLine(file.content, spec.js) };
}
function makeDraft(p, file) {
  const patch = makePatch(p, file);
  if (patch.already) return { already: true, summary: patch.summary, expected_visible: patch.expected_visible };
  const diff = fullDiff(file.path, file.content, patch.content, file.sha);
  if (applyFullDiff(file.content, diff) !== patch.content) throw new Error("internal diff validation failed");
  return {
    already: false,
    code_draft: {
...
}\nasync function autoPrepare(c, p) {
  const a = await activeTarget(c);
  p.status = "approved";
  p.file_path = a.effective.path;
  p.patch_draft = { target_file: a.effective.path, current_sha: a.effective.sha, intent: p.request, risk: "low", created_at: now() };
  const d = makeDraft(p, a.effective);
  if (d.already) {
    p.status = "already_applied";
    p.expected_visible = d.expected_visible;
    p.auto_pipeline = { ok: true, already: true, summary: d.summary, checked_at: now() };
    return { mode: "already", summary: d.summary, active: a };
  }
  p.code_draft = d.code_draft;
  p.expected_visible = d.code_draft.expected_visible;
  p.code_approved_at = now();
  p.status = "code_approved";
  p.auto_pipeline = { ok: true, summary: d.code_draft.summary, next: `/apply_confirm ${p.id}`, checked_at: now() };
  return { mode: "ready", summary: d.code_draft.summary, active: a };
}
async function ensureDraft(c, p) {
  const a = await activeTarget(c);
  const stale = !p.code_draft || p.code_draft.target_file !== a.effective.path || p.code_draft.current_sha !== a.effective.sha;
  ...
  p.file_path = a.effective.path;
  p.patch_draft = p.patch_draft || { target_file: a.effective.path, current_sha: a.effective.sha, intent: p.request, risk: "low", created_at: now() };
  p.code_draft = d.code_draft;
  p.expected_visible = d.code_draft.expected_visible;
  p.code_approved_at = p.code_approved_at || now();
  p.status = "code_approved";
  p.auto_rebuilt_at = now();
  return { active: a, rebuilt: true, already: false };
}
async function verifyPost(env, c, propId) {
  const props = await store(env, "proposals");
  const p = findProp(props, propId);
  const local = health();
  const pub = await publicHealth(c);
  let resultOk = false;
  let expected = "—";
  let filePath = "—";
  if (p) {
    expected = p.expected_visible || p.code_draft?.expected_visible || expectedForRequest(p.request || p.title)?.visible || "—";
    const a = await activeTarget(c);
    filePath = a.effective.path;
    resultOk = expected !== "—" && helpHas(a.effective.content, expected);
    p.post_apply_verify = { checked_at: now(), runtime_ok: true, result_ok: resultOk, expected_visible: expected, file: filePath, public: pub };
    p.status = resultOk ? "verified" : "partial";
    await save(env, "proposals", props, 50);
  }
  return [
    `🧪 Post-apply verify${p ? ` ${p.id}` : ""}:`,
    `- proposal: ${p ? p.status : "not found"}`,
    `- local version: ${local.version}`,
    `- runtime: PASS ✅`,
    `- expected visible: ${expected}`,
    `- checked file: ${filePath}`,
    `- proposal result: ${resultOk ? "PASS ✅" : "PARTIAL/FAIL ⚠️"}`,
    `- public /health: ${pub.ok ? "OK ✅" : `optional fail (${pub.http}) ⚠️`}`
  ].join("\n");
}
...
async function handleCommand(env, c, m) {
  ...
  if (command === "/help") return send(c, chatId, [
    "/start /help /status",
    "Development stage: " + VERSION,
    "/health_check /deploy_check /post_apply_verify id",
    "/self /self_set текст",
    ...
    "/patch_preview id /code_preview id /code_show id",
    "/apply_check id /code_approve id /apply_confirm id /apply_status id"
  ].join("\n"));
  if (command === "/status") {
    const tasks = await store(env, "tasks"), mem = await store(env, "memories"), props = await store(env, "proposals");
    return send(c, chatId, [
      "📡 MiniSkynet Core v4 status",
      `- version: ${VERSION}`,
      "- runtime: single file, no onion imports",
      "- auto propose pipeline: active",
      "- scoped help insert: active",
      "- proposal-specific verify: active",
      "- apply rebuild: active",
      "- owner guard: fixed",
  ...
  if (command === "/health_check" || command === "/deploy_check") { const pub = await publicHealth(c); return send(c, chatId, [`🩺 Health check v4.7:`,`- version: ${VERSION}`,`- scoped_help_insert: active ✅`,`- proposal_specific_verify: active ✅`,`- public /health: ${pub.ok ? "OK ✅" : `optional fail (${pub.http}) ⚠️`}`].join("\n")); }
  ...
  if (command === "/plan") return send(c, chatId, "🗺 Plan:\n1. Проверить v4.7 proposal-specific verify\n2. Проверить новый self-apply marker\n3. Вернуть обычный think");
  ...
  if (command === "/next") return send(c,chatId,"⏭ Next:\nПроверить /post_apply_verify после v4.7 и сделать новый marker-test");
  ...
  if (command === "/propose") {
    const props=await store(env,"proposals"); const p={id:makeId("prop"),status:"pending",title:clip(args,90),request:args,created_at:now()}; props.push(p);
    try { const r=await autoPrepare(c,p); await save(env,"proposals",props,50); if(r.mode==="already") return send(c,chatId,[`📦 Auto proposal ${p.id}`,"✅ Уже применено.",`- ${r.summary}`,"Apply не нужен."].join("\n")); return send(c,chatId,[`📦 Auto proposal ${p.id}`,"✅ approve: auto",`✅ patch: ${p.patch_draft.target_file}`,`✅ code: ${p.code_draft.summary}`,`✅ expected: ${p.expected_visible}`,"✅ apply_check: ok","✅ code_approve: auto","","Финальная запись в GitHub всё ещё требует одну команду:",`/apply_confirm ${p.id}`].join("\n")); }
...
async function handleTelegram(request, env) {
  ...
  if (msg.command) {
    if (!COMMANDS.has(msg.command)) {
      await send(c, msg.chatId, `Не знаю команду ${msg.command}. /help — список. Модель не вызываю.`);
      return json({ ok: true, unknown_command: true });
    }
    await handleCommand(env, c, msg);
    return json({ ok: true, command: msg.command, version: VERSION });
  }
  await send(c, msg.chatId, "Core v4.7 online. Proposal-specific verify active. Финальная запись через /apply_confirm. /help");
